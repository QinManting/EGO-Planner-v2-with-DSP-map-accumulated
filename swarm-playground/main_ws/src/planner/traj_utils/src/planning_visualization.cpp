...
void PlanningVisualization::displayMultiOptimalPathList(
    vector<vector<Eigen::Vector3d>> optimal_trajs,
    const double scale)  // zxzxzx
{
  ...
  for (int id = 0; id < (int)optimal_trajs.size(); id++) {
    ...
    displayMarkerList(optimal_list_pub, optimal_trajs[id], scale, color,
                      id + 10, false);
    ros::Duration(0.001).sleep();
    last_nums++;
  }
...
void PlanningVisualization::displayFailedList(Eigen::MatrixXd failed_pts,
                                              int id) {
...
void PlanningVisualization::displayIntermediatePt(std::string type,
                                                  Eigen::MatrixXd& pts, int id,
                                                  Eigen::Vector4d color) {
