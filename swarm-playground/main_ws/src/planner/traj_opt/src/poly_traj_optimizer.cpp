    ...
    if (result == lbfgs::LBFGS_CONVERGENCE ||
      ...
      if (!flag_swarm_too_close) {
        if (finelyCheckAndSetConstraintPoints(segments_nouse, jerkOpt_,
          ...
          flag_success = true;
          PRINTF_COND(
              "\033[32miter=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5."
              "3f\n\033[0m",
          ...
          restart_nums++;
          PRINTF_COND(
              "\033[32miter=%d,time(ms)=%5.3f, fine check collided, keep "
              "optimizing\n\033[0m",
              iter_num_, time_ms);
        }
      } else {
        PRINTF_COND(
            "Swarm clearance not satisfied, keep optimizing. "
  ...
  for (size_t i = 0; i < segment_ids.size(); i++) {
    ...
    for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j) {
      ...
      int Astar_id = a_star_pathes[i].size() / 2,
          last_Astar_id;  // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
...
/* multi-topo support */
std::vector<ConstraintPoints> PolyTrajOptimizer::distinctiveTrajs(
  ...
  for (int i = 0; i < seg_upbound; i++) {
    ...
    exit_multi_loop2:;
      ...
      for (int j = occ_start_id; j <= occ_end_id; j++) {
        ...
        if (grid_map_->getInflateOccupancy(base_pt_reverse))  // Search outward.
        {
          ...
          if (l > l_upbound) {
            ROS_WARN_COND(VERBOSE_OUTPUT,
                          "Can't find the new base points at the opposite "
                          "within the threshold. i=%d, j=%d",
                          i, j);
    ...
    exit_multi_loop3:;
      ...
      if (grid_map_->getInflateOccupancy(base_pt_reverse))  // Search outward.
      {
        ...
        if (l > l_upbound) {
          ROS_WARN_COND(VERBOSE_OUTPUT,
                        "Can't find the new base points at the opposite within "
                        "the threshold, 2. i=%d",
                        i);
  ...
  if (seg_upbound ==
      0)  // After the erase operation above, segment legth will decrease to 0 again.
  {
    std::vector<ConstraintPoints> oneSeg;
    oneSeg.push_back(cps_);
    return oneSeg;
  ...
  for (int i = 0; i < max_traj_nums; i++) {
    ...
    while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.cp_size) {
      ...
      if (seg_id >= seg_upbound || cp_id < segments[seg_id].first ||
        ...
        ROS_ERROR(
            "Shold not happen!!!!, cp_id=%d, seg_id=%d, "
