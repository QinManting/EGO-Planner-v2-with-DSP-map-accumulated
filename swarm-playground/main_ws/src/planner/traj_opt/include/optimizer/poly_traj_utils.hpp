...
class Piece {
 ...
 public:
  ...
  // GaaiLam
  inline bool project_pt(const Eigen::Vector3d& pt, double& tt,
...
class Trajectory {
 ...
 public:
  ...
  inline bool project_pt(const Eigen::Vector3d& pt, int& ii, double& tt,
                         Eigen::Vector3d& pro_pt) {
  ...
  inline Eigen::Vector3d getPoswithIdxRatio(
...
class MinJerkOpt {
 ...
 private:
  ...
  inline void addTimeIntPenalty(const Eigen::VectorXi cons,
                                ...
                                const Eigen::Vector3d ci, double& cost,
      ...
      for (int j = 0; j < innerLoop; j++) {
        ...
        for (int k = 0; k < K; k++) {
          ...
          if (violaPos > 0.0) {
            ...
            violaPosPenaD *= 3.0;
            gdC.block<6, 3>(i * 6, 0) += omg * step * ci(0) * violaPosPenaD *
                                         beta0 * outerNormal.transpose();
            gdT(i) += omg * (ci(0) * violaPosPenaD * alpha *
                                 outerNormal.dot(vel) * step +
 ...
 public:
  ...
  inline void generate(const Eigen::MatrixXd& inPs, const Eigen::VectorXd& ts) {
    if (inPs.cols() == 0) {
      ...
      coeffMatReversed.col(5) =
          0.5 * (tailPVA.col(2) - headPVA.col(2)) * t3_inv -
          3.0 * (headPVA.col(1) + tailPVA.col(1)) * t4_inv +
          6.0 * (tailPVA.col(0) - headPVA.col(0)) * t5_inv;
      coeffMatReversed.col(4) =
          (-tailPVA.col(2) + 1.5 * headPVA.col(2)) * t2_inv +
          (8.0 * headPVA.col(1) + 7.0 * tailPVA.col(1)) * t3_inv +
          15.0 * (-tailPVA.col(0) + headPVA.col(0)) * t4_inv;
      coeffMatReversed.col(3) =
          (0.5 * tailPVA.col(2) - 1.5 * headPVA.col(2)) * t1_inv -
  ...
  inline void getGrad2TP(EIGENVEC& gdT, EIGENMAT& gdInPs) {
    solveAdjGradC(gdC);
    addPropCtoT(gdC, gdT);
