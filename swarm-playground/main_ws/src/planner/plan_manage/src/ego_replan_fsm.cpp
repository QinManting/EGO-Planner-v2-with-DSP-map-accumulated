...
void EGOReplanFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/ground_height_measurement", enable_ground_height_measurement_,
           false);
  ...
  mandatory_stop_sub_ = nh.subscribe(
      "mandatory_stop", 1, &EGOReplanFSM::mandatoryStopCallback, this);

  /* Use MINCO trajectory to minimize the message size in wireless communication */
  broadcast_ploytraj_pub_ =
      nh.advertise<traj_utils::MINCOTraj>("planning/broadcast_traj_send", 10);
  broadcast_ploytraj_sub_ = nh.subscribe<traj_utils::MINCOTraj>(
      "planning/broadcast_traj_recv", 100,
      &EGOReplanFSM::RecvBroadcastMINCOTrajCallback, this,
      ros::TransportHints().tcpNoDelay());

  poly_traj_pub_ =
      nh.advertise<quadrotor_msgs::polynomial>("planning/trajectory", 1);
  data_disp_pub_ =
      nh.advertise<traj_utils::DataDisp>("planning/data_display", 100);
  heartbeat_pub_ = nh.advertise<std_msgs::Empty>("planning/heartbeat", 10);
  ground_height_pub_ =
      nh.advertise<std_msgs::Float64>("/ground_height_measurement", 10);
  // polynomial_pub = nh.advertise<traj_utils::polynomial>("/reference/polynomial", 1);

  if (target_type_ == TARGET_TYPE::MANUAL_TARGET) {
    waypoint_sub_ =
        nh.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback, this);
  } else if (target_type_ == TARGET_TYPE::PRESET_TARGET) {
    trigger_sub_ =
        nh.subscribe("/traj_start_trigger", 1, &EGOReplanFSM::triggerCallback,
                     this);  //当前使用
...
void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state,
  ...
  cout << "[" + pos_call + "]"
       ...
       << ", from " + state_str[pre_s] + " to " + state_str[int(new_state)]
       << endl;
}
...
find_ij_start:;
  ...
  for (size_t i = i_start; i < i_end; ++i) {
    for (size_t j = j_start; j < pts_chk[i].size(); ++j) {
      ...
      for (size_t id = 0; id < planner_manager_->traj_.swarm_traj.size();
           id++) {
        ...
        double t_X = t + (info->start_time -
                          planner_manager_->traj_.swarm_traj.at(id).start_time);
        if (t_X > 0 &&
            t_X < planner_manager_->traj_.swarm_traj.at(id).duration) {
          ...
          if (dist < allowed_dist) {
            ROS_WARN(
                "swarm distance between drone %d and drone %d is %f, too "
                "close!",
      ...
      if (dangerous) {
        ...
        {
          ROS_INFO("Plan success when detect collision. %f",
                   t / info->duration);
...
bool EGOReplanFSM::callEmergencyStop(Eigen::Vector3d stop_pos) {
  ...
  quadrotor_msgs::polynomial poly_msg;
...
bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp) {
  ...
  if (success) {
    ...
    int i_end =
        floor(planner_manager_->traj_.global_traj.duration / step_size_t);
    vector<Eigen::Vector3d> gloabl_traj(i_end);
    for (int i = 0; i < i_end; i++) {
      gloabl_traj[i] =
          planner_manager_->traj_.global_traj.traj.getPos(i * step_size_t);
    }
  ...
  if (planner_manager_->grid_map_->getInflateOccupancy(final_goal_)) {
    ...
    for (double t = planner_manager_->traj_.global_traj.duration; t > 0;
      ...
      if (!planner_manager_->grid_map_->getInflateOccupancy(pt)) {
        ...
        {
          ROS_INFO(
              "Current in-collision waypoint (%.3f, %.3f %.3f) has been "
              "modified to (%.3f, %.3f %.3f)",
...
void EGOReplanFSM::waypointCallback(const quadrotor_msgs::GoalSetPtr& msg) {
  ...
  ROS_INFO("Received goal: %f, %f, %f", msg->goal[0], msg->goal[1],
           msg->goal[2]);
...
void EGOReplanFSM::poseCallback(
...
void EGOReplanFSM::RecvBroadcastMINCOTrajCallback(
    const traj_utils::MINCOTrajConstPtr& msg) {
  ...
  if (planner_manager_->traj_.swarm_traj.size() > recv_id &&
      ...
      msg->start_time.toSec() -
          ...
          0) {
    ROS_WARN(
        "Received drone %d's trajectory out of order or duplicated, abandon "
        "it.",
        (int)recv_id);
    return;
  }
  ...
  if (abs((t_now - msg->start_time).toSec()) > 0.25) {
        ...
        10.0)  // 10 seconds offset, more likely to be caused by unsynced system time.
    {
      ...
      ROS_ERROR(
          "Time stamp diff: Local - Remote Agent %d = %fs, swarm time seems "
          "not synchronized, abandon!",
  ...
  /* Fill up the buffer */
  if (planner_manager_->traj_.swarm_traj.size() <= recv_id) {
    for (size_t i = planner_manager_->traj_.swarm_traj.size(); i <= recv_id;
         i++) {
      LocalTrajData blank;
      blank.drone_id = -1;
  ...
  if (msg->start_time.toSec() <=
      planner_manager_->traj_.swarm_traj[recv_id]
  ...
  headState << msg->start_p[0], msg->start_v[0], msg->start_a[0],
      msg->start_p[1], msg->start_v[1], msg->start_a[1], msg->start_p[2],
      msg->start_v[2], msg->start_a[2];
  tailState << msg->end_p[0], msg->end_v[0], msg->end_a[0], msg->end_p[1],
      msg->end_v[1], msg->end_a[1], msg->end_p[2], msg->end_v[2], msg->end_a[2];
  ...
  Eigen::MatrixXd cps_chk =
      MJO.getInitConstraintPoints(5);  // K = 5, such accuracy is sufficient
  bool far_away = true;
  for (int i = 0; i < cps_chk.cols(); ++i) {
    if ((cps_chk.col(i) - odom_pos_).norm() <
        planner_manager_->pp_.planning_horizen_ * 4 /
            3)  // close to me that can not be ignored
    {
      far_away = false;
      break;
    }
  ...
  if (!far_away ||
      !have_recv_pre_agent_)  // Accept a far traj if no previous agent received
  {
    ...
    planner_manager_->traj_.swarm_traj[recv_id].start_time =
        msg->start_time.toSec();
    planner_manager_->traj_.swarm_traj[recv_id].duration =
        trajectory.getTotalDuration();
    planner_manager_->traj_.swarm_traj[recv_id].start_pos =
        trajectory.getPos(0.0);
    planner_manager_->traj_.swarm_traj[recv_id].des_clearance =
        msg->des_clearance;
    ...
    if (!have_recv_pre_agent_) {
      if ((int)planner_manager_->traj_.swarm_traj.size() >=
          planner_manager_->pp_.drone_id) {
    ...
    planner_manager_->traj_.swarm_traj[recv_id].drone_id =
        -1;  // Means this trajectory is invalid
  }
}

void EGOReplanFSM::polyTraj2ROSMsg(quadrotor_msgs::polynomial& poly_msg,
                                   traj_utils::MINCOTraj& MINCO_msg) {
  ...
  MINCO_msg.start_p[0] = vec(0), MINCO_msg.start_p[1] = vec(1),
  MINCO_msg.start_p[2] = vec(2);
  vec = data->traj.getVel(0);
  MINCO_msg.start_v[0] = vec(0), MINCO_msg.start_v[1] = vec(1),
  MINCO_msg.start_v[2] = vec(2);
  vec = data->traj.getAcc(0);
  MINCO_msg.start_a[0] = vec(0), MINCO_msg.start_a[1] = vec(1),
  MINCO_msg.start_a[2] = vec(2);
  vec = data->traj.getPos(data->duration);
  MINCO_msg.end_p[0] = vec(0), MINCO_msg.end_p[1] = vec(1),
  MINCO_msg.end_p[2] = vec(2);
  vec = data->traj.getVel(data->duration);
  MINCO_msg.end_v[0] = vec(0), MINCO_msg.end_v[1] = vec(1),
  MINCO_msg.end_v[2] = vec(2);
  vec = data->traj.getAcc(data->duration);
  MINCO_msg.end_a[0] = vec(0), MINCO_msg.end_a[1] = vec(1),
  MINCO_msg.end_a[2] = vec(2);
...
bool EGOReplanFSM::measureGroundHeight(double& height) {
  if (planner_manager_->traj_.local_traj.pts_chk.size() <
      3)  // means planning have not started
  {
    return false;
  }
